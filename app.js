...
const S={
  ...
  syncConfigured:false,syncLastPending:0,syncLastState:null
};
...
function patientDisplayDate(value){
  if(value===null||value===undefined||value==='')return '';
  const months=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
  let y,m,d;
  if(value instanceof Date&&!isNaN(value)){y=value.getFullYear();m=value.getMonth()+1;d=value.getDate()}
  else{
    const s=String(value).trim();
    let x=s.match(/^(\d{4})-(\d{1,2})-(\d{1,2})(?:[T\s].*)?$/);
    if(x){y=+x[1];m=+x[2];d=+x[3]}
    if(!y&&(x=s.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/))){d=+x[1];m=+x[2];y=+x[3]}
    if(!y&&(x=s.match(/^(\d{1,2})[-\s]([A-Za-z]{3,9})[-\s](\d{4})$/))){
      const mi=months.map(z=>z.toLowerCase()).indexOf(x[2].slice(0,3).toLowerCase());
      if(mi>=0){d=+x[1];m=mi+1;y=+x[3]}
    }
    if(!y){
      const parsed=new Date(s);
      if(!isNaN(parsed)){y=parsed.getFullYear();m=parsed.getMonth()+1;d=parsed.getDate()}
    }
  }
  if(!y||!m||!d||m<1||m>12||d<1||d>31)return String(value);
...
function localPatientByKey(key){return S.today.find(p=>String(p._key)===String(key))||null}
function syncBeforeSnapshot(p){return p?JSON.parse(JSON.stringify(p)):null}
function syncCacheAndRender(){
  S.today.sort(patientSort);cacheTodaySession();populateModalityFilter();renderToday();
}
function applyOptimisticOp(op,{render=true}={}){
  if(!op)return;
  if(op.kind==='register'){
    if(!localPatientByKey(op.recordKey)){
      const d=op.payload?.patient||{};
      S.today.push({
        Name:d.name||'',RN:d.rn||'',Age:d.age||'',Sex:d.sex||'',Race:d.race||'',Ward:d.ward||'',
        Diagnosis:d.diagnosis||'',Operation:d.operation||'',Mode:d.mode||'',
        'Date Commenced':d.dateCommenced||'',"Doctor's name":d.doctorName||'',Remark:d.remark||'',
        Status:'New patient','Discharge Date':'',_key:op.recordKey,_status:'New patient',
        _roundCompleted:false,_roundOutcome:'',_dischargedToday:false,_detailLoaded:true,
        _syncPending:true,_syncOperationId:op.operationId,_optimisticRegistration:true
      });
    }else{
      const p=localPatientByKey(op.recordKey);p._syncPending=true;p._syncOperationId=op.operationId;
    }
  }else if(op.kind==='savePatient'){
    const p=localPatientByKey(op.recordKey);if(!p)return;
    const status=String(op.payload?.status||'Continue review'),discharged=status==='Discharged';
    p.Remark=String(op.payload?.remark??'');p.Status=status;p._status=status;
    p._roundCompleted=status!=='New patient';p._roundOutcome=status==='New patient'?'':status;
    p._dischargedToday=discharged;p['Discharge Date']=discharged?patientDisplayDate(new Date()):'';
    p._syncPending=true;p._syncOperationId=op.operationId;p._syncError='';
  }else if(op.kind==='editPatientDetails'){
    const p=localPatientByKey(op.recordKey);if(!p)return;
    const d=op.payload?.patient||{};
    const map={name:'Name',rn:'RN',age:'Age',sex:'Sex',race:'Race',ward:'Ward',diagnosis:'Diagnosis',operation:'Operation',mode:'Mode',dateCommenced:'Date Commenced',doctorName:"Doctor's name"};
    Object.keys(map).forEach(k=>{if(d[k]!=null)p[map[k]]=d[k]});
    p._syncPending=true;p._syncOperationId=op.operationId;p._syncError='';
  }
  if(S.current&&String(S.current._key)===String(op.recordKey))S.current=localPatientByKey(op.recordKey)||S.current;
  if(render)syncCacheAndRender();
}
function revertOptimisticOp(op){
  const current=localPatientByKey(op.recordKey);
  if(current&&current._syncOperationId&&current._syncOperationId!==op.operationId)return false;
  if(op.kind==='register'){
    const i=S.today.findIndex(p=>String(p._key)===String(op.recordKey));
    if(i>=0&&S.today[i]._optimisticRegistration)S.today.splice(i,1);
  }else if(op.before){
    const i=S.today.findIndex(p=>String(p._key)===String(op.recordKey));
    if(i>=0)S.today[i]={...op.before,_syncPending:false,_syncOperationId:'',_syncError:''};
  }
  if(S.current&&String(S.current._key)===String(op.recordKey))S.current=localPatientByKey(op.recordKey)||null;
  syncCacheAndRender();return true;
}
function onSyncConfirmed(op,result){
  const p=localPatientByKey(op.recordKey);
  // Never allow an older acknowledgement to overwrite a newer optimistic edit.
  if(p&&p._syncOperationId===op.operationId){
    if(result?.patient)Object.assign(p,result.patient);
    p._syncPending=false;p._syncOperationId='';p._syncError='';p._optimisticRegistration=false;
    syncCacheAndRender();
  }else if(op.kind==='register'&&!p&&result?.patient){
    updateLocalPatient(result.patient);
  }
}
async function onSyncConflict(op,response){
  const p=localPatientByKey(op.recordKey);
  if(p&&p._syncOperationId!==op.operationId){await APSOptimisticSync.discard(op.operationId);return}
  if(response?.conflictType==='duplicate-rn'){
    revertOptimisticOp(op);
    const r=response.result||{},d=(r.duplicates||[]).map(x=>`${x.name||'Patient'} (${x.ward||'ward unknown'})`).join(', ');
    const rn=op.payload?.patient?.rn||'';
    const ok=await askConfirm({title:'RN already active in APS',message:`Another active APS patient has RN ${rn}${d?' — '+d:''}. Save this RN anyway?`,ok:'Save anyway',cancel:'Cancel',danger:true});
    await APSOptimisticSync.discard(op.operationId);
    if(ok){
      const cur=localPatientByKey(op.recordKey);
      await APSOptimisticSync.enqueue({
        kind:'editPatientDetails',recordKey:op.recordKey,
        payload:{patient:op.payload.patient,confirmDuplicate:true},
        before:syncBeforeSnapshot(cur),coalesceKey:'details:'+op.recordKey
      });
      toast('Patient details updated locally · syncing','success',1500);
    }
  }
}
async function onSyncPermanentError(op,err){
  const recordGone=!!err?.recordGone||err?.code==='record-not-found'||/patient record could not be found|record could not be found/i.test(String(err?.message||''));
  if(recordGone){
    const key=String(op?.recordKey||'');
    await APSOptimisticSync.discardForRecord(key);
    const i=S.today.findIndex(p=>String(p._key)===key);
    if(i>=0)S.today.splice(i,1);
    if(S.current&&String(S.current._key)===key){
      S.current=null;S.patientOriginal=null;S.editOriginal=null;
      if($('#patientDialog')?.open){
        replaceOverlayWithViewState();
        $('#patientDialog').close();
      }
    }
    hideSaveError();
    syncCacheAndRender();
    toast('Patient no longer exists in the master list and was removed from this device.','success',2600);
    return;
  }
  const reverted=revertOptimisticOp(op);
  showSaveError(`Change not synced — ${err.message}`,()=>APSOptimisticSync.retryAll());
  if(reverted)toast('Master Sheet rejected a pending change; local view restored.','error',2600);
}
let syncBadgeTimer=null;
function renderSyncState(s){
  S.syncLastState=s;S.syncLastPending=s.pending||0;
  const b=$('#syncBadge');if(!b)return;
  clearTimeout(syncBadgeTimer);
  b.classList.remove('hidden','syncing','pending','error','synced');
  if(s.failed||s.conflicts){
    b.textContent=`! ${s.failed+s.conflicts} sync issue${s.failed+s.conflicts===1?'':'s'}`;b.classList.add('error');
  }else if(s.syncing){
    b.textContent=`↻ Syncing${s.pending?` ${s.pending}`:''}`;b.classList.add('syncing');
  }else if(s.pending){
    b.textContent=navigator.onLine?`${s.pending} pending`:`${s.pending} pending offline`;b.classList.add('pending');
  }else{
    b.textContent='✓ Synced';b.classList.add('synced');
    syncBadgeTimer=setTimeout(()=>b.classList.add('hidden'),1200);
  }
}
function setupOptimisticSync(){
  if(!window.APSOptimisticSync)return;
  if(!S.syncConfigured){
    APSOptimisticSync.configure({
      apiUrl:S.apiUrl,getToken:()=>S.token,
      onApply:op=>applyOptimisticOp(op),
      onReplay:ops=>{ops.forEach(op=>applyOptimisticOp(op,{render:false}));syncCacheAndRender()},
      onConfirmed:onSyncConfirmed,onConflict:onSyncConflict,onError:onSyncPermanentError,
      onAuthExpired:()=>{if(S.token){toast('Session expired — pending changes are kept safely for retry after login.','error',2600);lockApp()}},
      onStateChange:renderSyncState
    });
    S.syncConfigured=true;
  }else APSOptimisticSync.retryAll();
}
async function replayPendingOps(){
  if(!S.syncConfigured||!window.APSOptimisticSync)return;
  try{await APSOptimisticSync.replayPending()}catch(e){}
}
async function queuePatientSave(p,status,remark){
  if(!p)throw new Error('Patient is not available.');
  setupOptimisticSync();
  return APSOptimisticSync.enqueue({
    kind:'savePatient',recordKey:p._key,
    payload:{remark:String(remark??p.Remark??''),status},
    before:syncBeforeSnapshot(p),coalesceKey:'save:'+p._key
  });
}
...
async function loadToday({preserveScroll=false,force=false,background=false}={}){
  ...
  try{
    ...
    $('#todayDate').textContent=new Date().toLocaleDateString(undefined,{weekday:'short',day:'numeric',month:'short',year:'numeric'});
    populateModalityFilter();renderToday();cacheTodaySession();await replayPendingOps();
...
function cardHtml(p,round){return `<div class="card patient-card ${patientVisualState(p)}" data-sync-pending="${p._syncPending?'true':'false'}" data-key="${esc(p._key)}"><div class="patient-top"><div><div class="patient-name">${esc(p.Name)}</div><div class="patient-meta">RN ${esc(p.RN||'—')} · ${esc(p.Age||'—')} ${esc(p.Sex||'')}</div></div>${round?cardRoundState(p):`<div class="chips">${statusChip(p)}</div>`}</div><div class="chips">${p.Mode?`<span class="chip">${esc(p.Mode)}</span>`:''}${p['Date Commenced']?`<span class="chip">${esc(patientDisplayDate(p['Date Commenced']))}</span>`:''}</div>${p.Operation?`<div class="patient-meta"><b>Operation:</b> ${esc(p.Operation)}</div>`:''}${p.Remark?`<div class="remark">${esc(p.Remark)}</div>`:'<div class="remark muted">No remark</div>'}<div class="card-quick-actions"><button class="btn secondary remark-action" data-action="remark" data-key="${esc(p._key)}">Add/Edit Remark</button>${isDischarged(p)?`<button class="btn cancel-mini" data-action="cancel" data-key="${esc(p._key)}">Cancel discharge</button>`:`<button class="btn secondary" data-action="continue" data-key="${esc(p._key)}">Continue Review</button><button class="btn discharge-mini" data-action="discharge" data-key="${esc(p._key)}">Discharge</button>`}</div></div>`}
...
function renderList(el,patients,round){if(!patients.length){el.innerHTML='<div class="empty">No matching patients.</div>';return}el.innerHTML=wardGroups(patients).map(([w,ps])=>wardShell(w,ps,'list',ps.map(p=>`<div class="patient-node"><div class="compact-row ${patientVisualState(p)}" data-sync-pending="${p._syncPending?'true':'false'}" data-key="${esc(p._key)}"><div>${p['Date Commenced']?`<div class="list-date-commenced">${esc(patientDisplayDate(p['Date Commenced']))}</div>`:''}<div class="n">${esc(p.Name)}</div><div class="m">RN ${esc(p.RN||'—')} · ${esc(p.Mode||'—')}${p.Remark?' · '+esc(p.Remark):''}</div></div><div>${round?listRoundState(p):statusChip(p)}</div></div></div>`).join(''))).join('');bindWardDetails(el);bindPatientClicks(el)}
...
async function savePatientDetails(ev,{confirmDuplicate=false,identityConfirmed=false}={}){
  ...
  const old={...S.current},identityMsg=identityChangeMessage(old,data);
  ...
  try{
    hideSaveError();setupOptimisticSync();
    await APSOptimisticSync.enqueue({
      kind:'editPatientDetails',recordKey:old._key,
      payload:{patient:data,confirmDuplicate:!!confirmDuplicate},
      before:syncBeforeSnapshot(old),coalesceKey:'details:'+old._key
    });
    S.current=localPatientByKey(old._key)||S.current;S.editOriginal=null;
    toast('✓ Updated locally · syncing','success',1400);
  ...
  }catch(e){showSaveError('Could not queue patient update — '+e.message,()=>savePatientDetails(null,{confirmDuplicate,identityConfirmed:true}))}
...
async function directStatusAction(key,status){
  const p=localPatientByKey(key);if(!p)return;
  ...
  try{
    hideSaveError();await queuePatientSave(p,status,p.Remark||'');
    toast(discharge?'✓ Discharged locally · syncing':'✓ Review completed locally · syncing','success',1350);
  }catch(e){showSaveError('Could not queue change — '+e.message,()=>directStatusAction(key,status))}
}
async function directCancelDischarge(key){
  ...
  if(!ok)return;
  try{
    hideSaveError();await queuePatientSave(p,'Continue review',p.Remark||'');
    toast('✓ Discharge cancelled locally · syncing','success',1350);
  }catch(e){showSaveError('Could not queue change — '+e.message,()=>directCancelDischarge(key))}
}
async function saveCurrent(goNext){
  ...
  try{
    hideSaveError();await queuePatientSave(S.current,status,$('#detailRemark').value);
    S.current=localPatientByKey(oldKey)||S.current;
    S.patientOriginal={remark:S.current.Remark||'',status:isDischarged(S.current)?'Discharged':'Continue review'};
    toast('✓ Saved locally · syncing','success',1200);
    if(!goNext){
  ...
  }catch(e){showSaveError('Could not queue change — '+e.message,()=>saveCurrent(goNext))}
...
async function cancelDischarge(){
  ...
  try{
    await queuePatientSave(S.current,'Continue review',$('#detailRemark').value);
    toast('✓ Discharge cancelled locally · syncing','success',1350);replaceOverlayWithViewState();$('#patientDialog').close();S.patientOriginal=null;
  }catch(e){showSaveError('Could not queue change — '+e.message,()=>cancelDischarge())}
}

async function registerPatient(ev){
  ev.preventDefault();const form=ev.target,f=new FormData(form),patient=Object.fromEntries(f.entries());
  ...
  const btn=form.querySelector('button[type="submit"]');btn.disabled=true;btn.textContent='Registering…';
  try{
    hideSaveError();setupOptimisticSync();
    const op=await APSOptimisticSync.enqueueRegistration(patient);
    toast('✓ Patient registered locally · syncing','success',1500);
    form.reset();showView('Today');
    const p=localPatientByKey(op.recordKey);if(p)updateLocalPatient(p);
  }catch(e){showSaveError('Could not queue registration — '+e.message,()=>registerPatient(ev))}
...
function initMeds(){
  ...
  $('#medCategory').innerHTML='<option value="">All categories</option>'+cats.map(c=>`<option>${esc(c)}</option>`).join('');
}
...
function renderMeds(){
  ...
  const indices=[];for(let i=0;i<S.medications.length;i++){const m=S.medications[i];if(cat&&m.category!==cat)continue;if(q&&!S.medSearchIndex[i].includes(q))continue;indices.push(i)}
...
$('#syncBadge').onclick=()=>{if(window.APSOptimisticSync){APSOptimisticSync.retryAll();toast('Retrying pending changes…','success',1200)}};
$('#backToTop').onclick=()=>window.scrollTo({top:0,behavior:'smooth'});
window.addEventListener('scroll',()=>{saveScroll();$('#backToTop').classList.toggle('show',window.scrollY>520)},{passive:true});
window.addEventListener('online',()=>{setConnection(true);if(S.token){setupOptimisticSync();APSOptimisticSync?.retryAll();if(currentViewName()==='Today')setTimeout(()=>loadToday({preserveScroll:true,background:true}),450)}});window.addEventListener('offline',()=>setConnection(false));
...
function bootstrap(){
  ...
  if(S.token){
    ...
    if(Date.now()-S.lastActivity>ms)lockApp();
      ...
      $('#gate').classList.add('hidden');setupOptimisticSync();
