...
function addOmeRow(defaultKey='morphine_po'){
  ...
  row.innerHTML=`<select class="ome-form">${omeEligibleKeys().map(k=>`<option value="${k}"${k===defaultKey?' selected':''}>${esc(opOptionLabel(k))}</option>`).join('')}</select>
    <div class="ome-dose-wrap"><input class="ome-dose" type="number" min="0" step="any" inputmode="decimal" placeholder="Dose"><span class="ome-unit"></span></div>
