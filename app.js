...
function applyOptimisticOp(op,{render=true}={}){
  ...
  if(op.kind==='register'){
    ...
    p._dischargedToday=discharged;p['Discharge Date']=discharged?new Date().toLocaleDateString('en-GB'):'';
...
function onSyncPermanentError(op,err){
...
function cardHtml(p,round){return `<div class="card patient-card ${patientVisualState(p)}" data-sync-pending="${p._syncPending?'true':'false'}" data-key="${esc(p._key)}"><div class="patient-top"><div><div class="patient-name">${esc(p.Name)}</div><div class="patient-meta">RN ${esc(p.RN||'—')} · ${esc(p.Age||'—')} ${esc(p.Sex||'')}</div></div>${round?cardRoundState(p):`<div class="chips">${statusChip(p)}</div>`}</div><div class="chips">${p.Mode?`<span class="chip">${esc(p.Mode)}</span>`:''}${p['Date Commenced']?`<span class="chip">${esc(p['Date Commenced'])}</span>`:''}</div>${p.Operation?`<div class="patient-meta"><b>Operation:</b> ${esc(p.Operation)}</div>`:''}${p.Remark?`<div class="remark">${esc(p.Remark)}</div>`:'<div class="remark muted">No remark</div>'}<div class="card-quick-actions"><button class="btn secondary remark-action" data-action="remark" data-key="${esc(p._key)}">Add/Edit Remark</button>${isDischarged(p)?`<button class="btn cancel-mini" data-action="cancel" data-key="${esc(p._key)}">Cancel discharge</button>`:`<button class="btn secondary" data-action="continue" data-key="${esc(p._key)}">Continue Review</button><button class="btn discharge-mini" data-action="discharge" data-key="${esc(p._key)}">Discharge</button>`}</div></div>`}
